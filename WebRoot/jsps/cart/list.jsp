	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location="/CartItemServlet?method=batchDelet&cartItemIds="+cartItemId;	
			}
		} else {
...
function batchDelete(){
	...
	location ="/CartItemServlet?method=batchDelete&cartItemIds="+cartItemIdArray;
};
	...
	<c:choose>
		...
		<c:otherwise>
			<table width="95%" align="center" cellpadding="0" cellspacing="0">
				...
				<c:forEach items="${cartItemList}" var="cartItem">
					<tr align="center">
						...
						<td align="left" width="400px"><a
							href="<c:url value='/jsps/book/desc.jsp'/>"><span>${cartItem.book.bname
									}</span>
						</a></td>
						...
						<td><a class="jian" id="${cartItem.cartItemId}Jian"></a>
						...
						<td width="100px"><span class="price_n">&yen;<span class="subTotal" id="${cartItem.cartItemId}Subtotal">${cartItem.subtotal	}</span>		
						</span>
						</td>
						<td><a
				...
				<tr>
					...
					<td colspan="3" align="right" class="tdTotal"><span>总计：</span><span
						class="price_t">&yen;<span id="total"></span>
					</span></td>
				</tr>
				<tr>
					<td colspan="7" align="right"><a href="javascript:jiesuan();" id="jiesuan" class="jiesuan"></a></td>
				</tr>
			</table>
			<form id="jiesuanForm" action="<c:url value='/CartItemServlet'/>"
				...
				<input type="hidden" name="total" id="hiddenTotal" value="loadCartItems" />
			</form>
